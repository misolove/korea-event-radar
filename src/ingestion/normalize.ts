...
function looksNonTech(title: string): boolean {
  return /(바이오유스 캠프|계획기부|유산기부|힐링콘서트|강형욱|이대호|부동산|재테크|현금 파이프라인|커피포럼|바이어 응대|출판.*수출|비영리.*법령|근로기준법|경제.*클래스|돈 공부|킥스타터.*크라우드|음악.*작곡|한국위성|원폭|핵무기|WISE Vocabulary|경암바이오|의약학)/i.test(title);
}
